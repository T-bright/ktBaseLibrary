...
abstract class HttpConfigProxy {
     ...
     * 但是也有部分需求，可能这个异常不仅仅是给个提示，而是需要处理相应的业务逻辑，这个时候可以使用这个方法，拿到整个的BaseResponse<T>信息，去做特殊处理。
     *
     * 这里这么设计的原因：
     * 1：因为这种需要特殊处理的后台错误信息场景非常少，这种逻辑基本很少遇到；
     * 2：如果不这么设计，采用回调的方式，在协程里面就会多一层回调，使用这种方式，可以有效的避免多一层回调嵌套，让代码更加的清晰清爽。
     * 3：为什么不在 parseResponseData 方法里统一处理。因为如果http请求成功返回，T 不为空。此时走正常逻辑即可。但是http请求失败时，此时 T 为空，此时需要特殊
     *    处理的错误码逻辑将无法判断
     */
